...
export const getAllProducts =
  (Searchcategory = "", searchedName = "") =>
  async (dispatch) => {
    dispatch({ type: LOAD_ALL_PRODUCTS });
    try {
      const response = await axios.get(
        `http://localhost:5000/api/product/?category=${Searchcategory}&name=${searchedName}`
      );
...
export const addProduct = (newProduct, navigate) => async (dispatch) => {
  ...
  try {
    ...
    dispatch({
      ...
      payload: error.response.data.errors.name.message,
    });
...
export const getOneProduct = (id) => async (dispatch) => {
  try {
    const response = await axios.get(
      `http://localhost:5000/api/product/oneProduct/${id}`
    );
    console.log(response);
